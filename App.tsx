...
// ملاحظة هامة: يجب استبدال هذه القيم بالقيم التي تجدها في حسابك في Firebase
...
const App: React.FC = () => {
  ...
  const handleSaveSchedule = async (updated: Schedule) => {
    ...
    // عند الحفظ: تفريغ حقل البحث والعودة للقائمة الرئيسية
    setSearchQuery('');
    setActiveBranchTab(updated.branch);

    if (connectionStatus === 'online') {
      try { await setDoc(doc(db, "schedules", updated.id), updated); } catch (e) { saveLocally(updated); }
    } else { saveLocally(updated); }
    ...
    <Layout title={currentPage === 'admin-panel' ? "إدارة الدورة المائية" : "جدول ري فلاح"} onLogout={handleLogout} onNavigateToHome={() => { setCurrentPage(currentPage); setSearchQuery(''); }}>
      <div className="mb-6 space-y-2">
        <div onClick={() => setShowTechGuide(!showTechGuide)} className={`p-3 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black cursor-pointer transition-all ${connectionStatus === 'online' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100 animate-pulse'}`}>
          ...
          {connectionStatus === 'online' ? 'المزامنة السحابية تعمل بنجاح' : 'تنبيه: البيانات مخزنة محلياً فقط - اضغط هنا للإصلاح'}
        </div>
        ...
        {showTechGuide && (
          <div className="bg-white p-6 rounded-3xl border-2 border-indigo-100 shadow-xl space-y-4 animate-in slide-in-from-top duration-300">
            <h4 className="font-black text-indigo-950 flex items-center gap-2">
              ...
              طريقة ربط التطبيق بحسابك الخاص:
            </h4>
            <div className="text-xs text-slate-600 leading-relaxed space-y-3 font-bold">
              <p>1. اذهب لموقع <a href="https://console.firebase.google.com/" target="_blank" className="text-indigo-600 underline">Firebase Console</a> وأنشئ مشروعاً جديداً.</p>
              <p>2. فعل <b>Firestore Database</b> من قائمة Build.</p>
              <p>3. من تبويب <b>Rules</b>، تأكد أنها <code className="bg-slate-100 p-1">allow read, write: if true;</code>.</p>
            </div>
            <button onClick={() => setShowTechGuide(false)} className="w-full py-2 bg-indigo-50 text-indigo-600 rounded-xl font-black text-[10px]">إغلاق التعليمات</button>
          </div>
        )}
      </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-12 pb-20">
            {Object.entries(groupedSchedules[activeBranchTab] || {}).length === 0 ? (
              <div className="text-center py-20 opacity-30 font-black text-xl">لا توجد بيانات لهذا الخط حالياً</div>
            ) : (
